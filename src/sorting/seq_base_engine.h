#ifndef MEGAHIT_SEQ_BASE_ENGINE_H
#define MEGAHIT_SEQ_BASE_ENGINE_H
 ...
 */
class SeqBaseSequenceSortingEngine {
 public:
 ...
 public:
  SeqBaseSequenceSortingEngine(int64_t mem, int mem_flag, int n_threads, MPIEnviroment &mpienv)
      : host_mem_(mem), mem_flag_(mem_flag), n_threads_(n_threads), mpienv_(mpienv) {};

  SeqBaseSequenceSortingEngine(int64_t mem, int mem_flag, int n_threads)
      : host_mem_(mem), mem_flag_(mem_flag), n_threads_(n_threads) {};
      
  virtual ~SeqBaseSequenceSortingEngine() = default;
 ...
 protected:
  ...
  class OffsetFiller {
   public:
    OffsetFiller(SeqBaseSequenceSortingEngine *engine, unsigned start_bucket,
    ...
    void WriteNextOffset(unsigned bucket, int64_t full_offset) {
      ...
      int64_t diff = full_offset - prev_full_offsets_[bucket];//计算diff，prevfulloffsets里最初存的都是该线程分区第一个碱基相对全局pkd的偏移，所以这里diff表示在该线程分区中的相对偏移量
      int64_t index = bucket_index_[bucket]++; //获取该key在哪一个桶里，确定其索引，每写一个offset对应桶的index就加一，即后移一个
      engine_->WriteOffset(index, diff, full_offset);
      prev_full_offsets_[bucket] = full_offset;// ？ 每次更新对应bucket里面，前一个被写入元素的全局偏移，所以每次存的都是桶里在该线程分区中相对上一个被插入元素的偏移量！！
    }
   ...
   private:
    SeqBaseSequenceSortingEngine *engine_;
  ...
  class OffsetFetcher {
   public:
    OffsetFetcher(SeqBaseSequenceSortingEngine *engine, unsigned start_bucket,
                  unsigned end_bucket)
 ...
 private:
  void WriteOffset(size_t index, int64_t diff, int64_t full_offset) {
    if (diff > kDifferentialLimit) {
      int64_t bucket = kDifferentialLimit + 1 + AddSpecialOffset(full_offset); //这个+1什么作用？因为等于limit也是正常的，所以从limit下一个开始算
