...
void AssembleAndOutput(const HashMapper &mapper, const SeqPackage &read_pkg,
  ...
  MPIContigWriter local_contig_writer(output_file, mpienv.rank);

  int64_t num_ctg_mean = mapper.refseq().seq_count() / mpienv.nprocs;
  int64_t remain = mapper.refseq().seq_count() % mpienv.nprocs;
  int64_t start_index = mpienv.rank * num_ctg_mean + (mpienv.rank < remain ? mpienv.rank : remain);
  int64_t end_index = start_index + num_ctg_mean + (mpienv.rank < remain ? 1 : 0);
    
#pragma omp parallel for private(contig_graph, seq, contig_end, reads, \
                                 out_contigs, out_contig_infos)        \
    schedule(dynamic)
  for (uint64_t cid = start_index; cid < end_index; ++cid) {
    ...
    // 仅主线程检查并刷新 buffer
    if (omp_get_thread_num() == 0 && local_contig_writer.check_buf()) {
      local_contig_writer.MPIFileWrite();
    }
  }

  //clean buffer && allreduce
  local_contig_writer.MPIFileWrite();
  local_contig_writer.allreduce();
}
