...
class SequenceLibGA {
 public:
  SequenceLibGA(SeqPackageGA *data_holder, int64_t held_begin, int64_t held_end,
              unsigned max_read_length, bool is_paired, std::string description)
      : data_holder_(data_holder),
        held_begin_(held_begin),
        held_end_(held_end),
  ...
  SeqPackageGA::SeqView GetSequenceView(int64_t seq_id) const {
    assert(seq_id >= 0 && seq_id + held_begin_ < held_end_);
    return data_holder_->GetSeqView(seq_id + held_begin_);
  }
...
class SequenceLibCollection {
 public:
  ...
  void ReadGA(SeqPackageGA *pkg, MPIEnviroment& mpienv, bool reverse_seq = false);
 ...
 private:
  ...
  std::vector<SequenceLibGA> libs_GA_;
};
